<?php
class ControllerCatalogServices extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('catalog/services');
	...
	public function add() {
      ...
      if (isset($this->request->post['service_description']['price_new'])) {
				$data['service_description']['price_new'] = $this->request->post['service_description']['price_new'];
			} else {
				$data['service_description']['price_new'] = '';
			}

      if (isset($this->request->post['service_description']['time'])) {
				$data['service_description']['time'] = $this->request->post['service_description']['time'];
			} else {
				$data['service_description']['time'] = '';
			}

			if (isset($this->request->post['service_description']['parent_id'])) {
				$data['service_description']['parent_id'] = $this->request->post['service_description']['parent_id'];
			} else {
				$data['service_description']['parent_id'] = '';
			}
      ...
      if (isset($this->request->post['services_image'])) {
			...
			if (isset($this->request->post['services_related'])) {
				$data['services_related'] = $this->request->post['services_related'];
			} else {
				$data['services_related'] = array();
			}

			$this->model_catalog_services->addServices($data);
			...
			$this->response->redirect($this->url->link('catalog/services', 'user_token=' . $this->session->data['user_token'] . $url, true));
		}
	...
	public function edit() {
      ...
      if (isset($this->request->post['service_description']['time'])) {
			...
			if (isset($this->request->post['service_description']['device_id'])) {
				...
				$data['service_description']['device_id'] = array();
			}
			...
			$this->model_catalog_services->editServices($this->request->get['services_id'], $data);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected'])) {
			foreach ($this->request->post['selected'] as $services_id) {
				$this->model_catalog_services->deleteServices($services_id);
			}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('catalog/services', 'user_token=' . $this->session->data['user_token'] . $url, true)
		);

		$data['add'] = $this->url->link('catalog/services/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['delete'] = $this->url->link('catalog/services/delete', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		$services_total = $this->model_catalog_services->getTotalServices();

		$results = $this->model_catalog_services->getServicess($filter_data);

		foreach ($results as $result) {
			$data['services'][] = array(
				'services_id' => $result['services_id'],
				'name'        => $result['name'],
				'edit'        => $this->url->link('catalog/services/edit', 'user_token=' . $this->session->data['user_token'] . '&services_id=' . $result['services_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->link('catalog/services', 'user_token=' . $this->session->data['user_token'] . '&sort=agd.name' . $url, true);
		...
		$pagination->url = $this->url->link('catalog/services', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('catalog/services_list', $data));
	}
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['services_id'])) {
			$data['action'] = $this->url->link('catalog/services/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('catalog/services/edit', 'user_token=' . $this->session->data['user_token'] . '&services_id=' . $this->request->get['services_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('catalog/services', 'user_token=' . $this->session->data['user_token'] . $url, true);

		if (isset($this->request->get['services_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$services_info = $this->model_catalog_services->getServices($this->request->get['services_id']);
		}
		...
		if (isset($this->request->post['service_description'])) {
			...
			$data['service_description'] = $this->model_catalog_services->getServicesDescriptions($this->request->get['services_id']);
		} else {
			$data['service_description'] = array();
		}

    if ($data['service_description'] && $data['service_description']['category_services_id']) {
			$this->load->model('catalog/category_services');
			$category = $this->model_catalog_category_services->getCategoryServices($services_info['category_services_id']);
			$data['path'] = $category['name'];
		} else {
			$data['path'] = '';
		}
		...
		if ($data['service_description'] && $data['service_description']['device_id']) {
			...
			$category = $this->model_catalog_category->getCategory($services_info['device_id']);
		...
		if (isset($this->request->post['service_seo_url'])) {
			...
			$data['service_seo_url'] = $this->model_catalog_services->getServiceSeoUrls($this->request->get['services_id']);
		} else {
    ...
    if (isset($this->request->get['services_id'])) {
      $services_images = $this->model_catalog_services->getServicesImages($this->request->get['services_id']);
      ...
      $data['services_images'] = array();
		...
		if (isset($this->request->get['services_id'])) {
			$servicess = $this->model_catalog_services->getServicesRelated($this->request->get['services_id']);
		} else {
			$servicess = array();
		}

		$data['services_relateds'] = array();

		foreach ($servicess as $services_id) {
			$related_info = $this->model_catalog_services->getServices($services_id);

			if ($related_info) {
				$data['services_relateds'][] = array(
					'services_id' => $related_info['services_id'],
		...
		$results = $this->model_catalog_category_services->getCategoryServicess();

		foreach ($results as $result) {
			$data['categories'][] = array(
				'category_id' => $result['category_services_id'],
		...
		$results = $this->model_catalog_category->getDevicesServices();
		...
		$this->response->setOutput($this->load->view('catalog/services_form', $data));
	}

	protected function validateForm() {
		if (!$this->user->hasPermission('modify', 'catalog/services')) {
